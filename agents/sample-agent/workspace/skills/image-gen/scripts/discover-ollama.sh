...
if [[ "$FULL_SCAN" == "true" ]]; then
  ...
  # Add .130-.160 (common range including .145)
...
# Deduplicate and put localhost first (127.0.0.1, host.docker.internal)
ALIVE_UNIQ=($(printf '%s\n' "${ALIVE_HOSTS[@]}" | sort -u))
ALIVE_HOSTS=()
...
# Phase 2: Query each host for version + model list, build JSON via python3
...
# Phase 3: Assemble final catalog JSON via python3
